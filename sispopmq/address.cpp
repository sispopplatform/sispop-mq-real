...
std::string decode_pubkey(std::string_view& in, bool qr) {
    std::string pubkey;
    if (in.size() >= 64 && sispopmq::is_hex(in.substr(0, 64))) {
        pubkey = from_hex(in.substr(0, 64));
        in.remove_prefix(64);
    } else if (in.size() >= 52 && sispopmq::is_base32z(in.substr(0, 52))) {
        pubkey = from_base32z(in.substr(0, 52));
        in.remove_prefix(52);
    } else if (!qr && in.size() >= 43 && sispopmq::is_base64(in.substr(0, 43))) {
